...
class login_member extends CI_Controller
{
	...
	public function index(){
		...
		if($this->session->userdata('username')){
			...
			if($this->form_validation->run() == false){
				...
				$this->load->view('laundry/tampil_login_member');
	...
	private function _login(){
		...
		$cekuser=$this->m_laundry->cek_member($username)->row_array();
	...
	public function daftar(){
		...
		if($this->form_validation->run() == false){
			...
			$this->load->view('laundry/template/header',$data);
			$this->load->view('laundry/daftar_member');
			$this->load->view('laundry/template/footer');
		}else{
			$this->proses_daftar();
		}
	...
	private function proses_daftar(){
		...
		$cek=$this->m_laundry->cek_member($username)->row_array();
		...
		if($cek !=0){
			...
			$data=array('nama_admin'=>$username,
						...
						'hint'=>$hint);
			$tes=$this->m_laundry->daftar_member($data);
