...
class Autentifikasi extends CI_Controller
{
	public function index()
	{
		...
		if($this->form_validation->run() == false) {
			$data=array('judul'=>'Login',
						'user'=>'');
			//kata 'login' merupakan nilai dari variabel judul dalamarray $data dikirimkan ke view aute_header
	...
	private function _login()
	{
		...
		if($user){
			...
			if($user['is_active']==1){
					...
					$data=array(
					       ...
					       if($user['role_id']==1){
					       	...
					       	if($user['image']=='default.jpg'){
					       		$this->session->set_flashdata('pesan','<div class="alert alert-info alert-message" role="alert">Silahkan ubah profil anda untuk ubah poto profil</div>');
					       	}
					       	redirect('user');
					       }
				} else {
				...
				$this->session->set_flashdata('pesan','<div class="alert alert-danger alert-message" role="alert">USer belum diaktidasi!!</div>');
				redirect('autentifikasi');
			}
		} else {
	...
	public function registrasi()
	{
		...
		$this->form_validation->set_rules('nama','Nama Lengkap','required',array('required'=>'nama belum diisi !!'));
		...
		if($this->form_validation->run() == false){
			$data=array('judul'=>'Registrasi Member');
			$this->load->view('templates/aute_header',$data);
